...
`WARP_PREFERRED_LOCATION` принимает значения `auto`, `de`, `nl`, `pl`, `fi`, `gb`, `fr`, `us`, `ca`, `jp`, `sg` и `au`. Значение по умолчанию — `auto`. Это best-effort предпочтение, а не selector страны или Cloudflare colo. Это обычная переменная конфигурации деплоя, а не секрет. При необходимости `WARP_API_BASE_URL` позволяет заменить адрес внешнего API.

Файл `data/warpscout-report.txt` можно обновлять результатом локального сканирования WARPSCOUT. Во время сборки он преобразуется в статический registry. В интерфейсе доступны только страны, найденные в рабочей секции отчёта; группировка выполняется по `NODE LOCATION`, а `SEEN AS` сохраняется только для диагностики. Endpoint выбирается случайно среди быстрых endpoint с нулевой потерей пакетов. Registry действует семь дней и имеет fallback на Auto.
